...
export const initiatePayment = async (paymentInfo: TPaymentInfoProps) => {
  try {
    const response = await axios.post(config.payment_url!, {
      ...
      success_url: `http://localhost:5000/api/payments/conformation?transactionId=${paymentInfo.transactionId}&status=success`,
      fail_url: `http://localhost:5000/api/payments/conformation?status=faild`,
